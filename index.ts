...
async function routeRequest(
  ...
  if (req.method === 'GET' && pathname === '/health') {
    ...
    if (auth && req.method === 'GET' && pathname === '/api/custom-providers') {
      if (!isAdmin(auth)) {
        return errorResponse(req, 403, 'Solo administradores', 'forbidden');
      }
    ...
    if (auth && req.method === 'POST' && pathname === '/api/custom-providers') {
      if (!isAdmin(auth)) {
        return errorResponse(req, 403, 'Solo administradores', 'forbidden');
      }
      try {
        ...
        if (!body.name?.trim()) {
          return errorResponse(req, 400, 'name es obligatorio', 'validation_error');
        }
        if (!body.baseUrl?.trim()) {
          return errorResponse(req, 400, 'baseUrl es obligatorio', 'validation_error');
        }
    ...
    if (auth && req.method === 'PATCH' && customProviderMatch) {
      if (!isAdmin(auth)) {
        return errorResponse(req, 403, 'Solo administradores', 'forbidden');
      }
      try {
    ...
    if (auth && req.method === 'DELETE' && customProviderMatch) {
      if (!isAdmin(auth)) {
        return errorResponse(req, 403, 'Solo administradores', 'forbidden');
      }
      const deleted = deleteCustomProvider(customProviderMatch[1] ?? '');
      if (!deleted) {
        return errorResponse(req, 404, 'Proveedor no encontrado', 'not_found');
      }
